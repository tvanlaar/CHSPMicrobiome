...
BAR <- p + geom_boxplot(data = p$data, aes(x = site, y = value, color = NULL), alpha = 0.1) + theme(axis.title = element_text(face="bold"))

bar <- BAR + stat_compare_means(aes(label = ifelse(..p.signif.. < 0.05, ..p.signif.., "")), method = "wilcox.test", label.x = 1.3, label.y = Inf)

barsite<-bar+ stat_compare_means(aes(label = ..p.signif..), method = "wilcox.test",  label.x = 1.5) + xlab("Site")
